...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust:
            ...
            self.vel[0] += forward[0] * accelerate
            self.vel[1] += forward[1] * accelerate
    ...
    def update_thrust(self, hasThrust):
        self.thrust = hasThrust
        if self.thrust:
...
# key handlers for left/right arrow keys
...
def rock_spawner():
    ...
    a_rock = Sprite([random.randrange(WIDTH), random.randrange(HEIGHT)], 
                   ...
                   1, random.choice([0.05, -0.1]), asteroid_image, asteroid_info)
